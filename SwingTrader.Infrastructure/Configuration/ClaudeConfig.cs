...
public class ClaudeConfig
{
    ...
    // The high-value, GENUINELY LOW-FREQUENCY judgement calls run on Opus
    // (Mark, 13 Jul 2026): watchlist selection (both lists, weekly),
    // refinement analysis (monthly), Lab analysis/sweep explanation
    // (on-demand), filing-delta diff scoring (only on an actual filing
    // change), economic-link graph building (once per symbol per ~30 days).
    ...
    // (initially misrouted here 13 Jul 2026, corrected the same day once
    // an unexpectedly large API bill made clear it runs once per watchlist
    // symbol EVERY research cycle - the same cardinality as sentiment, not
    // a low-frequency synthesis call).
