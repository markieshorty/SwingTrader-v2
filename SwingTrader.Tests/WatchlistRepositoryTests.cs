...
public class WatchlistRepositoryTests
{
    ...
    [Fact]
    public async Task CreateWatchlistAsync_NonManualType_Throws()
    {
        // Only a manual (custom) watchlist can be created — the AI-managed and
        // Qualitative lists are system-owned.
        await using var db = CreateDb();
        var repo = new WatchlistRepository(db);
        await repo.SeedDefaultAsync(1);

        var act = async () => await repo.CreateWatchlistAsync(1, "Nope", WatchlistType.AiManaged, null);

        await act.Should().ThrowAsync<ValidationException>();
    }

    [Fact]
    public async Task CreateWatchlistAsync_SecondCustom_Throws()
    {
        // At most one custom watchlist per account.
        await using var db = CreateDb();
        var repo = new WatchlistRepository(db);
        await repo.SeedDefaultAsync(1);
        await repo.CreateWatchlistAsync(1, "Mine", WatchlistType.Manual, null);

        var act = async () => await repo.CreateWatchlistAsync(1, "Another", WatchlistType.Manual, null);
    ...
    [Fact]
    public async Task DeleteWatchlistAsync_QualitativeList_Throws()
    {
        // Only the user's custom (manual) list is deletable; the Qualitative
        // list is system-owned and can only be disabled.
        await using var db = CreateDb();
        var repo = new WatchlistRepository(db);
        await repo.SeedDefaultAsync(1);
        var now = DateTime.UtcNow;
        var qualitative = new Watchlist
        {
            AccountId = 1,
            Name = "Claude Qualitative",
            Type = WatchlistType.AiQualitative,
            IsEnabled = false,
            IsDefault = false,
            CreatedAt = now,
            UpdatedAt = now,
        };
        db.Watchlists.Add(qualitative);
        await db.SaveChangesAsync();

        var act = async () => await repo.DeleteWatchlistAsync(1, qualitative.Id);

        await act.Should().ThrowAsync<ValidationException>();
    }

    [Fact]
    public async Task DeleteWatchlistAsync_CustomManual_Succeeds()
    {
        await using var db = CreateDb();
        var repo = new WatchlistRepository(db);
        await repo.SeedDefaultAsync(1);
