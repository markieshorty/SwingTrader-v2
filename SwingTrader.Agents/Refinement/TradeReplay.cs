...
public static class ReplayEvaluator
{
    ...
    public static bool WouldTake(ReplayableTrade t, StrategyWeights w, decimal gateThreshold, bool excludeBreakout) =>
        WouldTake(t, w, gateThreshold, excludeBreakout ? BreakoutOnly : NoExclusions);
    ...
    public static bool WouldTake(ReplayableTrade t, StrategyWeights w, decimal gateThreshold, IReadOnlySet<SetupType> excludedSetups) =>
        !excludedSetups.Contains(t.Signal.SetupType)
        && Conviction(t.Signal, w) >= gateThreshold;
    ...
    public static Outcome Evaluate(IReadOnlyList<ReplayableTrade> trades, StrategyWeights w, decimal gateThreshold, bool excludeBreakout) =>
        Evaluate(trades, w, gateThreshold, excludeBreakout ? BreakoutOnly : NoExclusions);

    public static Outcome Evaluate(IReadOnlyList<ReplayableTrade> trades, StrategyWeights w, decimal gateThreshold, IReadOnlySet<SetupType> excludedSetups)
    {
        var kept = trades.Where(t => WouldTake(t, w, gateThreshold, excludedSetups)).ToList();
