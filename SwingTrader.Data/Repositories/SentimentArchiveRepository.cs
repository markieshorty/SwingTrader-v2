...
public class SentimentArchiveRepository(SwingTraderDbContext db) : ISentimentArchiveRepository
{
    ...
    public async Task<List<SentimentDailyScore>> GetScoresForSymbolsSinceAsync(
        IReadOnlyCollection<string> symbols, DateOnly from, CancellationToken ct = default)
    {
        ...
        return await db.SentimentDailyScores
            .Where(s => upper.Contains(s.Symbol) && s.Date >= from)
            .OrderByDescending(s => s.Date)
            .ToListAsync(ct);
    }

    public Task<List<SentimentDailyScore>> GetTopMoversSinceAsync(DateOnly from, int take, CancellationToken ct = default) =>
        db.SentimentDailyScores
            .Where(s => s.Date >= from && s.Score != 0m)
